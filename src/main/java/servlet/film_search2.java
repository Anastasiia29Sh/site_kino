...
@WebServlet("/film_search2")
public class film_search2 extends HttpServlet {
...
//        out.println(list_film3.size() + " " + countf + " " + start + " " + end);
    	...
    	request.setAttribute("name_str", "film_search2?type=" + type);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		eng_rus_genre.put("f", "фильм");
		eng_rus_genre.put("s", "сериал");
		eng_rus_genre.put("m", "мультфильм");
	    ...
	    String type = request.getParameter("type");
        String typee = eng_rus_genre.get(type);
        String name_head = (type.equals("f")) ? "ФИЛЬМЫ" : ((type.equals("s")) ? "СЕРИАЛЫ" : ((type.equals("m")) ? "МУЛЬТФИЛЬМЫ" : ""));
		...
		try {
            ...
            if(genre == null && country == null && year == null) {
    			...
    			if(typee.equals("фильм") || typee.equals("сериал") || typee.equals("мультфильм")) {
    				pS = con.prepareStatement("CALL film_search2(?, ?, ?, ?)");
    				pS.setString(1, genre);
        			pS.setString(2, country);
        			pS.setString(3, year);
        			pS.setString(4, typee);
                }else {
                	pS = con.prepareStatement("CALL film_search(?, ?, ?)");
                	pS.setString(1, genre);
        			pS.setString(2, country);
        			pS.setString(3, year);
                }
